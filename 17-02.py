...
while lava_heap:
    ...
    dirs_to_check = []
    if cons == 0 or cons >= 4:
        dirs_to_check.append((cur_d + 1) % 4)
        dirs_to_check.append((cur_d + 3) % 4)
    if cons < 10:
        dirs_to_check.append(cur_d)
    for ndir in dirs_to_check:
        nrow, ncol = row + deltas[ndir][0], col + deltas[ndir][1]
        ncons = 1 if cons == 0 or ndir != cur_d else cons + 1
        if ncol >= 0 and ncol < len(city_map[0]) and nrow >= 0 and nrow < len(city_map):
            if min_heat[ncons - 1][ndir][nrow][ncol] == -1:
                ...
                if nrow == len(city_map) - 1 and ncol == len(city_map[0]) - 1 and ncons >= 4:
                    print(ndist)
                    exit(0)
